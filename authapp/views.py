...
def attendance(request):
    ...
    if request.method == "POST":
        ...
        TrainedBy_name = request.POST.get('trainer')
        trainer_object = Trainer.objects.get(name=TrainedBy_name)
        query = Attendance(phonenumber=phonenumber, Login=Login,
                           Logout=Logout, SelectWorkout=SelectWorkout, TrainedBy=trainer_object)
        query.save()
    ...
    if request.method == "POST":
        ...
        try:
            if User.objects.get(email=email):
                messages.warning(request, "Email already is Taken")
                return redirect('/signup')
...
def enroll(request):
    ...
    existing_enrollment = Enrollment.objects.filter(
        PhoneNumber=request.user).first()

    if existing_enrollment:
        messages.warning(
            request, "You have already enrolled.")
